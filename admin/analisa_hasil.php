...
$sqlhapusnya = "TRUNCATE TABLE perangkingan";
$hasilnya = mysql_query($sqlhapusnya) or die(mysql_error());
?>
...
<div id="content">
		...
		<div class="panel-heading">
			<h6 class="panel-title">
				<tr align="right">
					...
					<th><?php echo "(" . $_POST['bobot_ket_miskin'] . ")"; ?></th>
			...
			<table class="table table-hover">
				<thead>
					<tr>
						...
						<th>C4. Keterangan Miskin (Benefit)</th>
				...
				<tbody>
					...
					?>
						<tr>
							...
							<td><?php echo round($min['min4'] / $dataku['keterangan_miskin'], 2); ?></td>
	...
	$bobot_ket_miskin		= $_POST['bobot_ket_miskin'];
					...
					?>
						<tr>
							...
							<td><?php echo round((($dataku['nilai_ipk'] / $max['max1']) * $bobot_nilai_ipk) +
									...
									(($min['min4'] / $dataku['keterangan_miskin']) * $bobot_ket_miskin) +
									(($dataku['usia'] / $max['max5']) * $bobot_usia), 2); ?></td>
						</tr>
					<?php
						$nik_mhs = $dataku['nik_mhs'];
						$saw = round((($dataku['nilai_ipk'] / $max['max1']) * $bobot_nilai_ipk) +
							(($min['min2'] / $dataku['penghasilan_ortu']) * $bobot_penghasilan_ortu) +
							(($dataku['semester'] / $max['max3']) * $bobot_semester) +
							(($min['min4'] / $dataku['keterangan_miskin']) * $bobot_ket_miskin) +
							(($dataku['usia'] / $max['max5']) * $bobot_usia), 2);


						$sqlnya = "insert into perangkingan (rank,nik_mhs,nilai) values ('','$nik_mhs','$saw')";
						$nilainya = mysql_query($sqlnya) or die(mysql_error());
					}	?>
				</tbody>
			</table>
		</div>
