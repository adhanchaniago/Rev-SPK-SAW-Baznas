...
<head>
  <title>Laporan Analisa Mahasiswa</title>
  <style>
    html,
    body {
      background: #eee;
...
<body>
  <div class="container">
    ...
    <br>
    <h3 align="center"><u>LAPORAN ANALISA HASIL</u></h3>
    <br>
    <br>
    <table>
      <thead>
      ...
      <tbody>
        <?php
        $ambil = (mysql_query("SELECT * FROM perangkingan a LEFT JOIN calonbeasiswa b ON a.nik_mhs=b.nik_mhs Order By a.nilai DESC"));
        while ($sql = mysql_fetch_array($ambil)) {
          // var_dump($sql);
        ?>
          <tr>
            <td><?php echo ++$no ?></td>
            <td><?php echo $sql['nilai'] ?></td>
